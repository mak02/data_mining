...
public class ClosedFHM {
  ...
  public int huicount=0;
  public int[] supportsaved = new int[1000];
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
	...
	private void fhm(int [] prefix,
		...
		for(int i=0; i< ULs.size(); i++){
			...
			if(X.sumIutils >= minUtility){
				// save to file
				writeOut(prefix, prefixLength, X, X.sumIutils);
			}
    ...
    for (int i = 0; i < huicount-1; i++) {
      ...
      if(allhui[i][j]==utilityList.item && flg != 0 && supportsaved[i]==utilityList.getSupport()){
        if(allhui[i][j+1]==0){
          flg = 4; // will be replaced
          willprint[i] = 0;
          willprint[huicount] = 1; // 1 means it will be printed.
        }
        else{
          flg = 5; // new will not be added
          willprint[huicount] = 0;
        }
      }
      else{
        willprint[huicount] = 1;
