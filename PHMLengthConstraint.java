...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		if(maximumLength>1){
			phm(itemsetBuffer, 0, null, listULForRecursion, minUtility);}
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
	...
	private void phm(int [] prefix,	int prefixLength, UtilityListPHM pUL, List<UtilityListPHM> ULs, int minUtility)
		...
		for(int i=0; i< ULs.size(); i++){
			...
			//Change Mayank
			if(X.sumIutils + X.sumRutils >= minUtility && prefixLength<maximumLength){
				// Change Mayank
				...
				for(int j=i+1; j < ULs.size(); j++){
					...
					// Change Mayank
					UtilityListPHM temp = construct(pUL, X, Y, minUtility, prefixLength + 2);
					// Change Mayank
					if(temp != null) {
						exULs.add(temp);
					}
	 ...
	 */
	 ...
	 // Change Mayank
	private UtilityListPHM construct(UtilityListPHM P, UtilityListPHM px, UtilityListPHM py, int minUtility, int pxyLength) {
		// Change Mayank

		// Change Mayank

		if(pxyLength>maximumLength){
			return null;
		}
